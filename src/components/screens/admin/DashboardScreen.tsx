...
const DashboardScreen: React.FC<DashboardScreenProps> = ({ localOrders, refreshOrders }) => {
  const pendingOrdersCount = localOrders.filter(order => order.status === 'pending').length;
  const readyOrdersCount = localOrders.filter(order => order.status === 'ready').length;
  const deliveredOrdersCount = localOrders.filter(order => order.status === 'delivered').length;
          ...
          <table className="min-w-full">
            ...
            <tbody>
              {localOrders.slice(0, 10).map(order => {
                ...
                const orderType = order.drinks.length > 0 && order.meals.length === 0 
                  ? "Boissons" 
