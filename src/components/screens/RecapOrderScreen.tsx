...
const RecapOrderScreen: React.FC<RecapOrderScreenProps> = ({
  ...
  return (
    <div className={`h-screen flex flex-col ${isDarkMode ? 'bg-gray-900' : 'bg-gray-100'}`}>
      ...
      <div className={`p-4 ${isDarkMode ? 'bg-gray-800' : 'bg-white'} ${isDarkMode ? 'border-gray-700' : 'border-gray-200'} border-t`}>
        ...
        <div className="mb-4">
          <input
            ...
            className={`w-full mb-2 h-12 text-lg px-3 rounded-md border ${isDarkMode ? 'bg-gray-700 border-gray-600 text-white' : 'border-gray-300 text-gray-800'}`}
        ...
        <button
          ...
          className={`w-full h-12 text-lg ${!hasItemsToSubmit ? 'bg-gray-400' : isDarkMode ? 'bg-blue-700 hover:bg-blue-800' : 'bg-blue-500 hover:bg-blue-600'} text-white rounded-md mb-2`}
        >
          Valider la commande
        </button>
        <button
          onClick={() => setCurrentScreen('splitPayment')}
          className={`w-full h-12 text-lg ${isDarkMode ? 'bg-green-700 hover:bg-green-800' : 'bg-green-500 hover:bg-green-600'} text-white rounded-md`}
        >
          Paiement séparé
        </button>
      </div>
